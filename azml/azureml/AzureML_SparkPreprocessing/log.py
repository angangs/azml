...
columns = ['Run ID', 'Step Run ID', 'Step ID', 'Step Name', 'Start Time (UTC)', 'End Time (UTC)', 'Error', 'Status']
...
if num_children == 3:
    for child_run in children_runs[:2]:
        ...
        if 'error' not in run_details.keys():
            values = [child_run.parent.id, run_details['runId'], counter, STEPS[counter],
                      pd.to_datetime(run_details['startTimeUtc']), pd.to_datetime(run_details['endTimeUtc']), 'No error', run_details['status']]
            temp_df = pd.DataFrame([values], columns=columns)
            df = df.append(temp_df, ignore_index=True)
        else:
            error = run_details['error']['error']['message']
            values = [child_run.parent.id, run_details['runId'], counter, STEPS[counter],
                      pd.to_datetime(run_details['startTimeUtc']), pd.to_datetime(run_details['endTimeUtc']), error, run_details['status']]
            temp_df = pd.DataFrame([values], columns=columns)
            df = df.append(temp_df, ignore_index=True)
        counter += 1
else:
...
logs_path = os.path.join(args.outputfolder, f'{args.process}Log_{parent_run_id}_{date}.csv')
